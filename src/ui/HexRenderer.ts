...
export function pixelToAxial(px: number, py: number): { q: number; r: number } {
...
export function getWorldRepeatVectors(worldW: number, worldH: number): { dx1: number; dy1: number; dx2: number; dy2: number } {
...
export function getWrapOffsets(worldW: number, worldH: number): { x: number; y: number }[] {
...
/** Dibuja un hexágono. */
function drawHexShape(ctx: CanvasRenderingContext2D, cx: number, cy: number, size: number): void {
  ctx.beginPath();
  for (let i = 0; i < 6; i++) {
...
function drawCity(
  ctx: CanvasRenderingContext2D,
  cx: number, cy: number,
  hexSize: number, color: string, level: number, name: string, zoom: number
): void {
...
function drawUnit(
  ctx: CanvasRenderingContext2D,
  cx: number, cy: number,
  hexSize: number, unit: { tribeColor: string; icon: string; healthPercent: number },
  zoom: number
): void {
