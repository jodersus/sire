...
import { CanvasRenderer, RenderState } from './CanvasRenderer.js';
import { InputHandler } from './InputHandler.js';
import { HexCell } from './HexRenderer.js';
import { preloadSprites } from './Sprites.js';
import { HUD, HUDState } from './HUD.js';
...
function generateTestMap(radius: number = 6): HexCell[] {
  const cells: HexCell[] = [];
  const terrains = ['plains', 'mountain', 'water', 'desert', 'forest'] as const;
  for (let q = -radius; q <= radius; q++) {
    for (let r = -radius; r <= radius; r++) {
      if (Math.abs(q + r) <= radius) {
        const terrain = terrains[Math.floor(Math.random() * terrains.length)];
        const cell: HexCell = { q, r, terrain };
...
function init(): void {
  ...
  const hudState: Partial<HUDState> = {
    turn: 1,
    resources: {
      Estrellas: 5,
      Población: 3,
  ...
  const input = new InputHandler(renderer.getCanvas(), camera, {
    onHexClick: (q, r, cell) => {
      ...
      for (const c of cells) {
        if (c.q !== q || c.r !== r) c.highlight = undefined;
      }
      hud.update({
  ...
  (window as any).__sire__ = { camera, cells, renderer, input, hud };
