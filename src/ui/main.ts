...
import { GameController, RenderHexCell } from '../game/GameController.js';
...
function setAuth(persistent: boolean): void {
  try {
    if (persistent) localStorage.setItem(AUTH_KEY, ACCESS_TOKEN);
    else sessionStorage.setItem(AUTH_KEY, ACCESS_TOKEN);
  } catch { /* noop */ }
}
...
function showSetupScreen(onStart: (numBots: number, humanTribe: string) => void): void {
  const container = document.getElementById('game-container');
  if (!container) return;
  container.innerHTML = '';
  ...
  const botBtns: HTMLButtonElement[] = [];
  ...
  for (const n of botOptions) {
    ...
    btn.addEventListener('click', () => {
      numBots = n;
      for (const b of botBtns) b.style.background = 'rgba(255,255,255,0.05)';
...
function startGame(numBots: number, humanTribe: string): void {
  const container = document.getElementById('game-container');
  if (!container) return;
  container.innerHTML = '';
  ...
  input = new InputHandler(renderer.getCanvas(), camera, {
    onHexClick: (q, r) => {
      ...
      if (!state.isHumanTurn || state.winner) return;
      game.clickHex(q, r);
    },
    onHexHover: (q, r) => {
  ...
  const cellMap = new Map<string, RenderHexCell>();
  const updateCellMap = () => {
    cellMap.clear();
  ...
  (input as any).getCellAt = (q: number, r: number) => {
  ...
  const origLoop = (renderer as any).loop;
  (renderer as any).loop = function(this: any) {
    updateCellMap();
    origLoop.call(this);
  };
  ...
  // Exponer para debug
  (window as any).__sire__ = { game, camera, renderer, hud };
