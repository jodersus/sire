...
export interface UnitDefinition {
  ...
  // Movimiento
  movement: number;
  // Terrenos permitidos: 'land' | 'water' | 'mountain' | 'all'
  terrain: string[];
  ...
  // Rango de ataque (1 = cuerpo a cuerpo)
  attackRange: number;

  // Expansible: si puede convertirse en otra unidad
  upgradeTo?: UnitType;
}
...
export function getUnit(type: UnitType): UnitDefinition {
  return UNITS[type];
}

/** Verificar si una unidad puede moverse por un terreno */
export function canTraverse(unit: UnitDefinition, terrain: string): boolean {
  return unit.terrain.includes('all') || unit.terrain.includes(terrain);
}

/** Verificar si una unidad puede atacar a distancia */
export function isRanged(unit: UnitDefinition): boolean {
