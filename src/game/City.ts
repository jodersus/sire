...
export interface Building {
  ...
  cost: ResourceCost;
  effect: string;
  // Efecto numérico si aplica
  bonus?: {
    type: 'production' | 'defense' | 'population' | 'sight';
    value: number;
  };
}
...
export interface City {
  id: string;
  name: string;
  ownerId: string; // jugador propietario
  ...
  // Unidades en cola de entrenamiento
  trainingQueue: {
    unitType: UnitType;
    turnsRemaining: number;
  }[];

  // Territorio: lista de celdas controladas
  territory: { x: number; y: number }[];

  // Producción acumulada (si la ciudad no se usó)
  storedProduction: number;
}

/** Crear una nueva ciudad nivel 1 */
export function createCity(id: string, name: string, ownerId: string, x: number, y: number): City {
  return {
    id,
    name,
    ownerId,
...
export function calculateTerritory(cx: number, cy: number, level: number): { x: number; y: number }[] {
...
export function getCityIncome(city: City): number {
...
export function queueTraining(city: City, unitType: UnitType, turns: number): void {
  city.trainingQueue.push({ unitType, turnsRemaining: turns });
}

/** Procesar cola de entrenamiento (llamar al final del turno) */
export function processTrainingQueue(city: City): UnitType[] {
  const completed: UnitType[] = [];
...
export function isInTerritory(city: City, x: number, y: number): boolean {
