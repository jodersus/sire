...
export class SphericalWorld extends HexGrid {
  ...
  addWrapped(c: AxialCoord, dq: number, dr: number): AxialCoord {
    return this.wrap({ q: c.q + dq, r: c.r + dr });
  }

  /** Vecinos con wrap-around: siempre 6 vecinos válidos. */
  neighborsWrapped(c: AxialCoord): AxialCoord[] {
    const dirs = [
      { q: 1, r: 0 },
      { q: 1, r: -1 },
      { q: 0, r: -1 },
   ...
   */
  distanceWrapped(a: AxialCoord, b: AxialCoord): number {
  ...
  same(c1: AxialCoord, c2: AxialCoord): boolean {
