...
export class MapGenerator {
  ...
  generate(registry: Registry): SphericalWorld {
    ...
    for (const c of world.allCoords()) {
      ...
      if (res) this.assignResources(res, terrain, moisture);
    }

    return world;
  }

  private pickTerrain(e: number, m: number): TerrainType {
    if (e < this.config.waterLevel * 0.5) return TerrainType.DEEP_WATER;
    if (e < this.config.waterLevel) return TerrainType.SHALLOW_WATER;
    if (e < this.config.waterLevel + 0.05) return TerrainType.SAND;
    if (e > this.config.mountainLevel + 0.1) return TerrainType.VOLCANO;
    if (e > this.config.mountainLevel) return TerrainType.MOUNTAIN;
    if (e > this.config.mountainLevel - 0.15 && m < 0.3) return TerrainType.HILL;
    if (m > 0.6) return TerrainType.FOREST;
