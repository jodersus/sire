...
import { getAvailableTechs } from '../game/Technologies.js';
export class HUD {
    camera;
    state = null;
    logMessages = [];
    ...
    drawSidePanel(ctx, width, height) {
        const w = 200;
        const top = 50;
        const p = 12;
        ...
        if (this.state.phase === TurnPhase.TECHNOLOGY) {
            ...
            for (const tech of techs.slice(0, 4)) {
                ...
                this.drawButton(ctx, width - w + p, y, w - p * 2, 24, `${tech.name} (${tech.cost}⭐)`, 'research_tech', tech.id);
                y += 30;
            }
            if (techs.length === 0) {
                ctx.fillStyle = '#64748b';
    ...
    drawButton(ctx, x, y, w, h, text, action, data) {
